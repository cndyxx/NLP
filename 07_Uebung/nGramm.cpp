...
float NGramm::calculateLaplaceDiscount(string prefix, string suffix){
    float probability;
    if (prefixMap.end() != prefixMap.find(prefix)) {
        auto suffixMap=prefixMap[prefix];
        if (suffixMap.end() != suffixMap.find(suffix)) {
            probability=(suffixMap[suffix]+1)/(prefixCount[prefix]+symbolCount.size());
        }
        else{
            probability=1/(prefixCount[prefix]+symbolCount.size());
        }
    }
    else{
        probability=0.0;
...
float NGramm::calculateEntropy(string textFile){
    ...
    for(int x = 1;x<n;x++){
        ...
        if(laplaceDiscounting){
            probability = calculateLaplaceDiscount(prefix, suffix);
        } else{
...
void NGramm::countSymbol(string symbol){
    bool inlist=false;
    for(auto &currsymbol:symbolCount){
    ...
    if(!inlist){
       symbolCount.push_back(symbol);
    }
}

void NGramm::setLaplaceDiscounting(bool value){
    laplaceDiscounting = value;
}